...
#include "cpu.h"
#include <chrono>
#include <conio.h> // Use <termios.h> + custom input on Linux
#include <iomanip>
#include <iostream>
#include <map>
#include <sstream>
...
void ClearScreen() {
    std::cout << "\x1B[2J\x1B[H"; // ANSI escape to clear screen and move cursor to top-left
}
...
int main() {
    ...
    while (true) {
        ...
        char key = _getch(); // Use termios on Linux
        if (key == ' ') {
            do {
                nes.cpu.clock();
