...
void cpu6502::reset() {
    absAdd = 0xFFFC;
    uint16_t lo = read(absAdd + 0);
    uint16_t hi = read(absAdd + 1);
...
void cpu6502::irq() {
    if(getFlag(I) == 0) {
        ...
        write(0x0100 + stkp, hi);
        stkp--;
        write(0x0100 + stkp, lo);
        stkp--;
...
uint8_t cpu6502::IND() {
    ...
    if(lo_ptr == 0x00FF) {
        absAdd = read(ptr & 0xFF00) << 8 | read(ptr + 0);
    } else {
        absAdd = read(ptr + 1) << 8 | read(ptr + 0);
    }
    return 0;
}

uint8_t cpu6502::IZX() {
    uint16_t add = read(pc);
    pc++;

    uint16_t lo = read((uint16_t)(add + (uint16_t)x) & 0x00FF);
    uint16_t hi = read((uint16_t)(add + (uint16_t)x + 1) & 0x00FF);
...
uint8_t cpu6502::IZY() {
    ...
    uint16_t lo = read(add & 0x00FF);
    uint16_t hi = read((add+1) & 0x00FF);
...
uint8_t cpu6502::RTI() {
    ...
    stkp ++;
    pc = (temp) | (read(0x100 + stkp) << 8);
    return 0;
}

uint8_t cpu6502::RTS() {
    stkp++;
    uint16_t pclo = read(0x0100 + stkp);
    stkp++;
    uint16_t pchi = read(0x0100 + stkp);
...
// Disassembly Function for Testing Emulation
